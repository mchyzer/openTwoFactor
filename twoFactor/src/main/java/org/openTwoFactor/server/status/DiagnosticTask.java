...
import org.openTwoFactor.server.j2ee.TwoFactorStatusServlet;
...
public abstract class DiagnosticTask {
  ...
  protected boolean ignoringDiagnostic() {
    ...
    boolean ignoreDiagnostics = TwoFactorServerConfig.retrieveConfig().propertyValueBoolean("status.diagnostic.ignore." + this.retrieveName(), false);
  ...
  public void appendSuccessTextLine(String text){
    ...
    this.retrieveSuccessText().append("SUCCESS ").append(this.retrieveName())
      .append(": ").append(text).append(TwoFactorStatusServlet.elapsedSuffix() + "\n");
  }
