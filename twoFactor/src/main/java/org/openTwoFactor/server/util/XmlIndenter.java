...
public class XmlIndenter {
  ...
  public String result() {
    ...
    return TwoFactorServerUtils.trim(this.result.toString());
  }
  ...
  private void newlineIndent() {
    this.result.append("\n").append(TwoFactorServerUtils.repeat("  ", this.currentNumberOfIndents));
  }
  ...
  static boolean textTag(String xml, int endTagIndex, String tagName, 
    ...
    if (TwoFactorServerUtils.equals("XML_COMMENT", tagName)) {
      return false;
    }
    if (TwoFactorServerUtils.equals(tagName, nextTagName) && isNextCloseTag) {
      return true;
    }
    return false;
  }
  ...
  public XmlIndenter(String theXml) {
    if (theXml != null) {
      this.xml = TwoFactorServerUtils.trimToEmpty(theXml);
    }
  }
