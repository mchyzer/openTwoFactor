 ...
 * @param <V> 
 */
public class TwoFactorCache<K,V> {
   ...
   * @param theCache
   */
  public TwoFactorCache(Cache theCache) {
    this.cache = theCache;
  }
   ...
   */
  public TwoFactorCache(String cacheName) {
    this(TfEhcacheController.ehcacheController().getCache(cacheName));
  }
   ...
   */
  public TwoFactorCache(String cacheName, int defaultMaxElementsInMemory, 
      boolean defaultEternal, int defaultTimeToIdleSeconds, 
      int defaultTimeToLiveSeconds, boolean defaultOverflowToDisk) {
    this(TfEhcacheController.ehcacheController().getCache(cacheName, true, defaultMaxElementsInMemory, defaultEternal, defaultTimeToIdleSeconds, defaultTimeToLiveSeconds, defaultOverflowToDisk));
    this.clear();
  }
  ...
  public synchronized Set<K> keySet() {
    List keyList = TwoFactorServerUtils.nonNull(this.cache.getKeys());
